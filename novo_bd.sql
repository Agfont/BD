...
-- 1.a)
CREATE OR REPLACE FUNCTION exclusividade_conflitos() AS $hierarquia$
DECLARE tipo_definido
BEGIN
    tipo_definido := (SELECT tipo FROM conflitos WHERE CodConf = NEW.CodConf);
    IF TG_OP = 'INSERT' AND tipo_definido IS NOT NULL THEN
        RAISE EXCEPTION "Erro: O conflito % já é do tipo %.", NEW.CodConf, tipo_definido;
    END IF;
    RETURN NEW;
END;
$hierarquia$
...
-- 1.c)
CREATE OR REPLACE FUNCTION divisao_min_1_chefe() AS $divisaoMin1$
BEGIN
...
-- 1.d)
CREATE OR REPLACE FUNCTION divisao_max_3_chefes() AS $divisaoMax3$
BEGIN
    IF (SELECT COUNT(codigo_chef) FROM chefes_militares 
...
-- 1.e)
CREATE OR REPLACE FUNCTION grupo_min_1_div() AS $grupoMinDiv$
BEGIN
    IF (TG_OP = 'DELETE') THEN
...
-- 1.f)
CREATE OR REPLACE FUNCTION conflito_min_2_grupos() AS $conflito2Grupos$
DECLARE grupos_part int;
BEGIN
    grupos_part := (SELECT COUNT(DINSTINCT grupo_armado_id) 
                    FROM EntPart
                    ORDER BY conflito_id);
...
-- 1.g)
CREATE OR REPLACE FUNCTION conflito_min_1_pais() AS $conflito1Pais$
BEGIN
    IF TG_OP = 'DELETE' AND
...
-- 1.h)
CREATE OR REPLACE FUNCTION num_baixas() AS $novasBaixas$
BEGIN
...
--1.h - Parte 2)
CREATE OR REPLACE FUNCTION id_divisoes() AS $idDiv$
DECLARE id int;
BEGIN
